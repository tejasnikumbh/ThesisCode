...
for dataset = 1:3
    ...
    for j = 1:num_iter
        ...
        for i = 1:k                                  %# for each fold
            ...
            pred_disc = classifier.predict(train_data(testIdx,:));
            
            %# train an SVM model over training instances
            classifier = ClassificationTree.fit(train_data(trainIdx,:), train_class(trainIdx));
            %# test using test instances
            pred_tree = classifier.predict(train_data(testIdx,:));
            
            %# train an SVM model over training instances
            classifier = ClassificationKNN.fit(train_data(trainIdx,:), train_class(trainIdx));
            %# test using test instances
            pred_knn = classifier.predict(train_data(testIdx,:));
           
            % Applying the Majority Rule - 1
            %{
            pred_3 = [pred_tree';pred_knn';pred_disc'];
            pred_3 = pred_3';
            pred = zeros(size(pred_knn,1),1);
            
            for example = 1:size(pred_knn,1)
                if(pred_3(example,1) == pred_3(example,2))
                    pred(example) = pred_3(example,1);
                else
                    pred(example) = pred_3(example,3);
                end
            end
            %}
            
            % Applying Majority Rule 2
            pred = pred_tree + pred_knn + pred_disc;
            pred = pred./3;
            for example = 1:size(pred_knn,1)
                if(pred(example)<0.25)
                    pred(example) = 0;
                else
                    pred(example) = 1;
                end
